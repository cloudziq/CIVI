...
func update_sun(progress):
	...
	if progress <= .33:
		moon.light_energy = lerp(.1, 0, (progress / .33))
		sun_color = sun_color_evening.linear_interpolate(sun_color_day, progress / .33)
		...
		sun.light_energy = lerp(.8, .2, (progress - .66) / .33)
		env.environment.ambient_light_energy = lerp(.8, .4, (progress - .66) / .33)
...
func update_moon(progress) -> void:
	...
	if progress <= .33:
		sun.light_energy = lerp(.2, .0, (progress / .33))
		...
		moon.light_energy = lerp(0.25, 0.1, (progress - 0.66) / 0.33)
